...
(function($) {
    ...
    $.fn.extend({
        manualExpandTextarea: function() {
            ...
            t.style.height = (t.scrollHeight > 350 ? 350 : t.scrollHeight) + "px", t.style.maxHeight = "", $(t).trigger("textareaChanged")
        },
    ...
    var SBF = {
        ...
        ajax_error: function(response, data = false) {
            ...
            if (SBApps.dialogflow.busy) {
                ...
                if (!admin && SBChat.conversation) {
                    SBF.ajax({
                        function: 'open-ai-send-fallback-message',
        ...
        null: function(obj) {
            if (typeof(obj) !== ND && obj !== null && obj !== 'null' && obj !== false && (obj.length > 0 || typeof(obj) == 'number' || typeof(obj.length) == ND) && obj !== ND) return false;
            else return true;
        },
                    ...
                    SBF.ajax({
                        ...
                        if (response && Array.isArray(response)) {
                            ...
                            if (SBF.setting('wp-users-system') == 'wp') {
                                SBApps.wordpress.ajax('wp-login', {
                                    user: email,
                                    password: password
                                });
                            }
                ...
                if (!response) {
                    ...
                    if (response.user_type) {
                        if (!admin && SBF.isAgent(response.user_type)) {
                            ...
                            activeUser(new SBUser(response, response.phone ? {
        ...
        event: function(name, parameters) {
            ...
            let webhooks_list = {
                ...
                SBNewMessagesReceived: 'new-messages',
        ...
        loadResource: function(src, js = false, onLoad = false, content = false) {
            ...
            if (src) {
                if (js) resource.src = src;
                else resource.href = src;
    ...
    class SBMessage {
        ...
        getCode(translation = false) {
            ...
            let message = translation && this.get('translation') ? this.get('translation') : (this.message ? this.message : '');
    ...
    class SBConversation {
        ...
        updateMessagesStatus(ids = false) {
            if (ids) {
                ...
                if (ids.length) {
                    SBF.ajax({
                        function: 'update-messages-status',
                        message_ids: ids
                    });
                }
            }
    ...
    var SBChat = {
        ...
        sendMessage: function(user_id = -1, message = '', attachments = [], onSuccess = false, payload = false, conversation_status_code = false) {
                        ...
                        if (response.length) {
                            ...
                            if (payload) {
                                ...
                                payload = {
                                    ...
                                    'original-message-language': language
                                };
                            }
                            if (response[0]) {
                                message = response[0];
            ...
            if (message || attachments.length || payload) {
                ...
                if (is_user) {
                    ...
                    chat.append((new SBMessage({
                        id: 'sending',
                        profile_image: (admin ? SB_ACTIVE_AGENT.profile_image : activeUser().image),
                        full_name: admin ? SB_ACTIVE_AGENT.full_name : activeUser().name,
                        creation_time: '0000-00-00 00:00:00',
                        message: message.replaceAll('<', '&lt;'),
                        user_type: (admin ? 'agent' : 'user')
                    })).getCode().replace('<div class="sb-time"></div>', `<div class="sb-time">${sb_('Sending')}<i></i></div>`));
                }
                ...
                if (audio_recorder_dom && audio_recorder_dom.sbActive()) {
                    ...
                    setTimeout(() => {
                        ...
                        if (source == 'wa' || audio_recorder_chunks.length) {
                            form.append('file', new File([source == 'wa' ? SBAudioRecorder.blob() : new Blob(audio_recorder_chunks, {
                                type: source == 'ig' ? 'audio/wav' : 'audio/mp3'
                            })], 'voice_message.' + (source == 'ig' ? 'wav' : 'mp3')));
                ...
                SBF.ajax({
                    ...
                    if (this.conversation) {
                        if (Array.isArray(response) && count > 0 && (!last_message || last_message.id != response[count - 1].id || last_message.message != response[count - 1].message || last_message.payload != response[count - 1].payload || last_message.attachments != response[count - 1].attachments)) {
                            ...
                            this.busy(false);
                            SBF.event('SBNewMessagesReceived', {
                                messages: messages,
                                conversation_id: this.conversation.id
                            });
                            if (tickets) {
        ...
        populateConversations: function(onSuccess = false) {
            if (!this.is_busy_populate && activeUser()) {
                ...
                activeUser().getConversations((response) => {
                    ...
                    this.finalizeInit();
                    SBF.event('SBPopulateConversations', {
        ...
        headerAgent: function(is_default_chatbot = false) {
            if (!admin && !tickets && !this.dashboard && this.conversation && (this.agent_id == -1 || (this.conversation.getLastMessage() && SBF.isAgent(this.conversation.getLastMessage().get('user_type')) && this.conversation.getLastMessage().get('user_id') != this.agent_id))) {
                ...
                agent = agent && SBApps.dialogflow.humanTakeoverActive() && SBPusher.active && !SBPusher.presenceCheck({
                    info: {
                        user_type: 'agent'
                    },
                    id: agent.user_id
                ...
                if (!agent && is_default_chatbot) {
                    agent = {
                        user_id: CHAT_SETTINGS.bot_id,
                        full_name: CHAT_SETTINGS.bot_name,
                        profile_image: CHAT_SETTINGS.bot_image
                    };
                }
                if (agent) {
        ...
        showArticles: function(id = false, is_category = false) {
            ...
            if (!id && CHAT_SETTINGS.articles_categories) {
                this.getArticleCategories((categories) => {
                    this.showArticles_(categories, panel, {
                ...
                this.getArticles(is_category ? false : id, (articles) => {
                    if (id && !is_category) {
                        ...
                        SBF.event('SBArticles', {
                            id: id,
                            articles: articles
                        });
                    } else {
                        this.showArticles_(articles, panel, {
                            id: id,
                            articles: articles
                        });
                    }
            ...
            if (chat_emoji.sbTogglePopup(button)) {
                if (!admin) {
                    chat_emoji.css({
                        ...
                        top: chat_editor.offset().top - window.scrollY - (tickets ? chat_editor.height() - 330 : 304)
                    });
                }
        ...
        slackMessage: function(user_id, full_name, profile_image, message, attachments = []) {
            ...
            SBF.ajax({
                ...
                SBF.event('SBSlackMessageSent', {
                    message: message,
                    conversation_id: conversation_id,
                    slack_channel: response[1]
                });
            });
        },
    ...
    var SBRichMessages = {
        ...
        submit: function(area, type, element) {
            if (!admin && !loading(element) && !this.is_busy) {
                ...
                let user_settings = activeUser() == false ? {
                    profile_image: '',
                    first_name: '',
                    last_name: '',
                    email: '',
                ...
                switch (type) {
                    ...
                    case 'registration':
                        ...
                        payload['event'] = 'update-user';
                        parameters = {
                            function: activeUser() ? 'update-user-and-message' : 'add-user-and-login',
                            settings: user_settings,
                            settings_extra: settings_extra,
                            payload: payload
                        };
                    ...
                    case 'buttons':
                        ...
                        if (type == 'chips') {
                            SBChat.sendMessage(activeUser().id, settings, [], false, {
                                id: rich_message_id,
                                event: 'chips-click',
                                result: settings
                            }, rich_message_id == 'sb-human-takeover' && input.index() == 0 ? 2 : false);
                        ...
                        switch (type) {
                            ...
                            case 'registration':
                                ...
                                if (CHAT_SETTINGS.wp_registration && user_settings.email && user_settings.password) {
                                    SBApps.wordpress.ajax('wp-registration', {
                                ...
                                setTimeout(() => {
                                    SBF.event('SBRegistrationForm', {
                                        ...
                                        conversation_id: SBChat.conversation ? SBChat.conversation.id : false,
                                        user: user_settings,
                                        extra: payload['rich-messages'][rich_message_id]['result']['extra']
                                    });
                                }, 5000);
                                break;
                            case 'buttons':
            ...
            table.find('[data-time]').each(function() {
                ...
                for (var i = 0; i < times.length; i++) {
                    if (times[i] == 'closed') {
                        ...
                        code += time.toLocaleTimeString([], {
                            hour: '2-digit',
                            minute: '2-digit'
                        }) + (i == 0 || i == 2 ? `<span>${sb_('to')}</span>` : i == 1 && times[i + 1] ? `<br />` : '');
                    }
                }
    ...
    var SBApps = {
        ...
        login: function() {
            if (this.is('wp') && typeof SB_WP_ACTIVE_USER != ND && CHAT_SETTINGS.wp_users_system == 'wp') {
                return [
                    [SB_WP_ACTIVE_USER, typeof SB_WP_AVATAR != ND ? SB_WP_AVATAR : ''], 'wp'
                ];
            }
            if (typeof SB_PERFEX_ACTIVE_USER != ND) {
                return [
                    [SB_PERFEX_ACTIVE_USER, SB_PERFEX_CONTACT_ID], 'perfex'
                ];
            }
                                        ...
                                        if (messages[i].payload) {
                                            ...
                                            if (payloads[2] in payload) {
                                                ...
                                                SBApps.woocommerce.updateCart(payload_value[0], payload_value[1], (response) => {
                                                    if (response) {
                                                        SBF.event('SBWoocommerceCartUpdated', {
                                                            'action': payload_value[0],
                                                            'product': payload_value[1]
                                                        });
                                                    }
                                                });
                                            }
                                            if (payloads[3] in payload && payload[payloads[3]] === true) {
                                                SBApps.wordpress.ajax('url', {
                                                    url_name: 'checkout'
                                                }, (response) => {
                                            ...
                                            if (payloads[5] in payload && payload[payloads[5]]) {
                                                SBF.ajax({
                                                    ...
                                                    if (payload[payloads[5]] == 'email' && activeUser().email) {
                                                        SBChat.sendEmail(payload.message ? payload.message : '', [
                                                            [response, response]
                                                        ], conversation.id);
    ...
    function initialize() {
        ...
        $(chat).on('click', '.sb-rich-woocommerce_button a, [href="#"].sb-card-btn', function(e) {
            ...
            if (product_ids.length) {
                ...
                SBApps.wordpress.ajax('button-purchase', {
        ...
        $(chat_editor).on('click', '.sb-btn-audio-clip', function() {
            ...
            if (audio_mp3 && typeof SBAudioRecorder === ND) {
                return $.getScript(SB_URL + '/vendor/lame.min.js', () => {
                    this.click();
                }, true);
            }
                    ...
                    audio_recorder_dom.on('click', '.sb-btn-clip-player', function() {
                        ...
                        if (is_pause) {
                            ...
                            if (audio_recorder_time_player[3]) {
                                ...
                                let audio = new Audio(URL.createObjectURL(audio_mp3 ? SBAudioRecorder.blob(false) : new Blob(audio_recorder_chunks, {
                                    type: 'audio/webm'
                                })));
        ...
        $(global).on('click', '.sb-input-image .image > .sb-icon-close', function(e) {
            SBF.ajax({
                function: 'delete-file',
                path: $(this).parent().attr('data-value')
            });
